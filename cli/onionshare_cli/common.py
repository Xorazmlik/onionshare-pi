...
class Common:
    ...
    def get_resource_path(self, filename):
        ...
        with importlib_resources.as_file(ref) as path:
            self.log("Common", "get_resource_path", f"fayl={filename}, yo'l={str(path)}")
            return str(path)
    def get_tor_paths(self):
        if self.platform == "Linux":
            ...
            if not os.path.exists(tor_path):
                self.log(
                    "Common", "get_tor_paths", f"tor.exe topilmadi: {tor_path}"
                )
...
class AutoStopTimer(threading.Thread):
    ...
    def run(self):
        self.common.log(
            "AutoStopTimer", f"Server {self.time} soniyadan so'ng yopiladi"
        )
